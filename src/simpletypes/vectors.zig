...
/// Three-part Vector, not to be confused with the SIMD @Vector or std::vector<>
pub const Vector3 = struct {
    ...
    pub inline fn badCross(lhd: Vector3, rhd: Vector3) Vector3 {
        return Vector3{ .x = lhd.y * rhd.z - lhd.z * rhd.y, .y = lhd.z * rhd.x - lhd.x * rhd.z, .z = lhd.y * rhd.x - lhd.x * rhd.y };
    }
    pub inline fn cross(self: Vector3, vect: Vector3) Vector3 {
        return Vector3{ .x = self.y * vect.z - self.z * vect.y, .y = self.z * vect.x - self.x * vect.z, .z = self.x * vect.y - self.y * vect.x };
    }
