...
pub const Chunk = struct { index: pst.pnt.Point3 = .{ .x = 0, .y = 0, .z = 0 }, heights: []u16, height_mod: u8, ground_sprite: *spt.Sprite, loaded: bool };

// map of all chunks arranged linearly
// TODO utilize allocation for large chunk maps
const chunk_map_size = 8;
var chunk_map = [_]Chunk{.{ .index = undefined, .heights = undefined, .height_mod = 0, .ground_sprite = undefined, .loaded = false }} ** (chunk_map_size * chunk_map_size);
...
pub fn loadChunk(index: pst.pnt.Point3) !*Chunk {
    if (chunkIndexIsValid(index)) {
        ...
        if (chunk.loaded == false) {
            ...
            std.debug.print("Loaded ({d}, {d}) Heights address:{x}", .{ chunk.index.x, chunk.index.y, @ptrToInt(&chunk.heights) });
...
pub fn getHeight(position: pst.Position) f32 {
    ...
    if (dec_x == 0 and dec_y == 0) {
        ...
        if (pos_x == 1 and pos_y == 1)
            return (getHeight(position.addVec(pst.vct.Vector3.init(1.0, 1.0, 0))) +
                getHeight(position.addVec(pst.vct.Vector3.init(1.0, -1.0, 0))) +
                getHeight(position.addVec(pst.vct.Vector3.init(-1.0, -1.0, 0))) +
                getHeight(position.addVec(pst.vct.Vector3.init(-1.0, 1.0, 0)))) / 4.0;
        if (pos_x == 1)
            return (getHeight(position.addVec(pst.vct.Vector3.init(1.0, 0, 0))) +
                getHeight(position.addVec(pst.vct.Vector3.init(-1.0, 0, 0)))) / 2.0;
        if (pos_y == 1)
            return (getHeight(position.addVec(pst.vct.Vector3.init(0.0, 1.0, 0))) +
                getHeight(position.addVec(pst.vct.Vector3.init(0.0, -1.0, 0)))) / 2.0;
    ...
    var origin = pst.vct.Vector3{ .x = dec_x, .y = dec_y, .z = 5.0 };
